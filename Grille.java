...
public class Grille {
	...
	/**
	* Indique si il est possible de jouer à une case précise
	*/

	public boolean isCoupPossible(int colonne, int ligne) {
		if(isCoupPossible(colonne))
		{
			if (ligne ==0 ){
			return grille[colonne][ligne]==Case.V;
			}
			return grille[colonne][ligne-1]!=Case.V && grille[colonne][ligne]==Case.V ;
		}
		else
		{
			return false;
	...
	/**
	 * retire le dernier coup joué dans la colonne spécifiée
	 * @param colonne
	 */
	public void RetirerCoup(int colonne) {
		for(int j=1;j<=Constantes.NB_LIGNES;j++)
		{
			if(grille[colonne][Constantes.NB_LIGNES -j] != Case.V)
			{
				grille[colonne][Constantes.NB_LIGNES -j]= Case.V;
				break;
			}
		}
